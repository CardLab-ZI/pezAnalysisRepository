...
char in = Serial1.read();
    ...
    else if (stateTH > 0)
    {
      ...
      {
        stateTH = 0;
       // DEBUG (v1h3, humidity-readout investigation): forward the raw line exactly as
       // received from the sensor, over the same primary Serial connection, for direct
       // viewing in Arduino IDE's Serial Monitor. Intended for use with the MATLAB GUI
       // OFF (single owner of the port at a time) -- run the Teensy standalone and drive
       // it with the same single-character commands runPezControl would otherwise send
       // (e.g. 'I' for IR intensity, 'Q' for the cooler setpoint) to reproduce low humidity.
        Serial.print("RAWTH: ");
        for (int dbgI = 0; dbgI < indexTH; dbgI++) Serial.write(buff[dbgI]);
        Serial.print("\r\n");
