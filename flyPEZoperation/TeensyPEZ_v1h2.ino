...
char in = Serial1.read();
    ...
    else if (stateTH > 0)
    {
      ...
      {
        stateTH = 0;
       // DEBUG (v1h2, humidity-readout investigation): forward the raw line exactly as
       // received from the sensor out the second USB-serial port (requires Tools > USB
       // Type > Dual Serial in the Arduino IDE), so it can be inspected independently of
       // MATLAB and independently of whether the parsing below succeeds or not.
        for (int dbgI = 0; dbgI < indexTH; dbgI++) SerialUSB1.write(buff[dbgI]);
        SerialUSB1.print("\r\n");
